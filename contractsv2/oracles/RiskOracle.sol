...
contract RiskOracle is AccessControl, ReentrancyGuard {
    ...
    function assessStrategyRisk(address strategy) external view returns (
        ...
        RiskAssessment memory assessment = currentRiskAssessments[strategy];
        bool isValid = assessment.valid && block.timestamp <= assessment.expiryTime;
    ...
    function getRiskAssessment(address protocol) external view returns (
        ...
        RiskAssessment memory assessment = currentRiskAssessments[protocol];
        isValid = assessment.valid && block.timestamp <= assessment.expiryTime;
        
        if (!isValid) {
            // Return conservative default assessment
            return (5000, 0, "UNKNOWN", 0, address(0), bytes32(0), false, 0, false);
        }
